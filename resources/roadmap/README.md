...
flowchart TB
  ...
  subgraph S0["00 · Foundations"]
    direction LR
    a1["Functional vs non-functional"] ~~~ a2["Estimation · QPS · storage"] ~~~ a3["Latency numbers"] ~~~ a4["Percentiles and the nines"]
  end

  subgraph S1["01 · Scaling the web tier"]
    direction LR
    b1["Vertical vs horizontal"] ~~~ b2["Load balancers · L4 · L7"] ~~~ b3["Stateless tier and sessions"] ~~~ b4["SPOF · redundancy · failover"] ~~~ b5["Multi-region · autoscaling"]
  end

  subgraph S2["02 · The data tier"]
    direction LR
    c1["SQL vs NoSQL"] ~~~ c2["Replication and lag"] ~~~ c3["Sharding and partition keys"] ~~~ c4["Hotspots and resharding"] ~~~ c5["Backups · search · geospatial"]
  end

  subgraph S3["03 · Caching and delivery"]
    direction LR
    d1["Cache-aside · write-through"] ~~~ d2["TTL · eviction · invalidation"] ~~~ d3["Stampede and hot keys"] ~~~ d4["CDN and cache headers"]
  end

  subgraph S4["04 · Asynchronous messaging"]
    direction LR
    e1["Queue vs publish-subscribe"] ~~~ e2["Partitions · ordering · consumer groups"] ~~~ e3["Delivery semantics · idempotency"] ~~~ e4["Retries · backoff · dead letter queues"] ~~~ e5["Fan-out · EDA · outbox · saga"]
  end

  subgraph S5["05 · Distributed systems theory"]
    direction LR
    f1["CAP and PACELC"] ~~~ f2["Consistency models"] ~~~ f3["Consensus · quorum · leader election"] ~~~ f4["Clocks · split brain · unique IDs"]
  end

  subgraph S6["06 · Reliability engineering"]
    direction LR
    g1["Timeouts · retries · circuit breakers"] ~~~ g2["Load shedding · rate limiting"] ~~~ g3["SLI · SLO · error budgets"] ~~~ g4["Observability and tracing"] ~~~ g5["Deploys · migrations · chaos"]
  end

  subgraph S7["07 · Communication and architecture"]
    direction LR
    h1["Monolith vs microservices"] ~~~ h2["REST · gRPC · GraphQL"] ~~~ h3["WebSocket · SSE · polling"] ~~~ h4["Gateways · discovery · versioning"]
  end

  subgraph S8["08 · Security and tenancy"]
    direction LR
    i1["AuthN vs AuthZ · OAuth · JWT"] ~~~ i2["Encryption in transit and at rest"] ~~~ i3["DDoS · WAF · input validation"] ~~~ i4["Multi-tenancy · PII · audit"]
  end

  subgraph S9["09 · Putting it together"]
    direction LR
    j1["The four-step framework"] ~~~ j2["Trade-offs out loud"] ~~~ j3["The classic catalogue"] ~~~ j4["Build the thing"]
  end

  S0 -->|"one server is slow and dies alone"| S1
  S1 -->|"the database is now the bottleneck"| S2
  S2 -->|"the same reads keep repeating"| S3
  S3 -->|"the request blocks on work it need not wait for"| S4
  S4 -->|"many machines now, and the network lies"| S5
  S5 -->|"so failure is the steady state"| S6
  S6 -->|"and the system became many services"| S7
  S7 -->|"with real users and real attackers"| S8
  S8 -->|"now assemble it in 45 minutes"| S9
```
